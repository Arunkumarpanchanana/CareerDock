...
**Goal:** Refactor the `/profile` page into a Structured Settings Hub with 3 grouped cards, CSS variable styling, persona selector, and plan tier display.

**Architecture:** Single client component rewrite. No new API routes, migrations, or components. Uses existing `Button`, `Input`, `Card`, `Badge`, `ReferralCard` from the app's design system.

**Tech Stack:** Next.js 16 (App Router), React 19, Tailwind CSS v4, Supabase Auth, Lucide icons

---

### Task 1: Rewrite the Profile Page

**Files:**
- Modify: `src/app/(protected)/profile/page.tsx` (full rewrite, 247 → ~280 lines)

- [ ] **Step 1: Write the new profile page component**
...
Expected: No TypeScript or lint errors. Build succeeds.

- [ ] **Step 3: Commit**

```bash
git add src/app/\(protected\)/profile/page.tsx
git commit -m "redesign: structured profile settings hub with persona selector and CSS variable styling"
```
