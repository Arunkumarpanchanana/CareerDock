            ...
            <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-4">
              {features.map(({ icon: Icon, title, desc, color, delay }) => (
                <StaggerItem key={title} delay={delay}>
                  <div className="group rounded-xl border border-slate-200 bg-white p-6 hover:-translate-y-1 hover:shadow-lg hover:border-blue-200 transition-all duration-300 cursor-default">
            ...
            <AnimatedSection>
              ...
              <div className="mt-4 flex items-center justify-center gap-3">
                <div className="flex -space-x-2">
                  {['bg-blue-500', 'bg-emerald-500', 'bg-violet-500'].map((bg) => (
                    <div key={bg} className={`h-10 w-10 rounded-full ${bg} ring-2 ring-white flex items-center justify-center text-white text-xs font-bold`}>
                      {bg === 'bg-blue-500' ? 'S' : bg === 'bg-emerald-500' ? 'J' : 'M'}
                    </div>
                  ))}
                </div>
