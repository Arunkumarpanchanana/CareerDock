...
export function FresherTemplate({ profile, data }: { profile: Profile | null; data: ResumeFormData }) {
  return (
    <div className="p-8" style={{ fontFamily: 'Georgia, "Times New Roman", serif' }}>
          ...
          {data.education.map((edu, i) => (
            <div key={i} className="mb-3">
              ...
              {edu.relevant_coursework && edu.relevant_coursework.length > 0 && (
                <div className="mt-1">
                  <span className="text-xs font-medium text-gray-500">Relevant Coursework: </span>
                  <span className="text-xs text-gray-600">{edu.relevant_coursework.join(', ')}</span>
                </div>
              )}
            </div>
          ))}
      ...
      {/* Projects — BEFORE experience for freshers */}
      ...
      {/* Experience — AFTER projects for freshers */}
